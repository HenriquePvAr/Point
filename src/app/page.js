...
const primeiroNome = (nome) => {
  if (!nome || typeof nome !== "string") return "Usuário";
  const n = nome.trim();
  if (!n) return "Usuário";
...
export default function Page() {
  ...
  const [meusConsumos, setMeusConsumos] = useState([]);
  ...
  const cores = temaEscuro
    ? {
        ...
        subHeader: "bg-[#333]",
        input: "bg-[#2c2c2c] text-white border-gray-600",
        sideActive: "bg-[#333] text-blue-400 border-l-blue-400",
      }
    : {
        bg: "bg-[#f5f5f5]",
        header: "bg-[#071d41]",
        ...
        sideActive: "bg-blue-50 text-[#1351b4] border-l-[#1351b4]",
      };
  ...
  function salvarContaNoDispositivo(userObj, listaAtual) {
    ...
    const base = (listaAtual || [])
      .map(normalizarConta)
      .filter((c) => c.email && c.email !== nova.email);
  ...
  function atualizarUltimoUso(emailUser, listaAtual = contasSalvas) {
    const novaLista = (listaAtual || [])
      .map(normalizarConta)
      .map((c) => (c.email === emailUser ? { ...c, lastUsed: Date.now() } : c));
  ...
  async function carregarDadosUsuario(userId, empresaId) {
    try {
      ...
      // 3. Consumos (Empresa)
      if (empresaId) {
        const resConsumo = await fetch(
          `/api/consumos?userId=${userId}&empresaId=${empresaId}`
        );
  ...
  useEffect(() => {
    ...
    let salvos = [];
    try {
      salvos = JSON.parse(localStorage.getItem("point_users") || "[]").map(
        normalizarConta
      );
    } catch {
      salvos = [];
    ...
    async function checarSessao() {
      try {
        ...
        if (data.success && data.user) {
          ...
          atualizarUltimoUso(data.user.email, listaAtualizada);
          toast.success(`Bem-vindo de volta, ${primeiroNome(data.user.nome)}!`);
        } else {
  ...
  useEffect(() => {
    const atualizarInterface = () => {
      ...
      const ultimaSaida = registrosDoTurno
        .filter((h) => h.tipo === "Saída")
        .at(-1);
  ...
  async function redefinirSenhaRecuperacao() {
    if (!codigoRecuperar || !novaSenhaRecuperar)
      return toast.warning("Preencha o código e a nova senha.");
  ...
  if (verificandoSessao) {
    return (
      <div className={`min-h-screen overflow-x-hidden flex flex-col items-center justify-center ${cores.bg}`}>
  ...
  if (!user || modalNovaSenha) {
    return (
      <div className={`min-h-screen overflow-x-hidden flex flex-col items-center justify-center font-sans ${cores.bg} p-4`}>
        {contaParaRemover && (
            ...
            <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md animate-scale-in">
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-[#1351b4] text-white font-bold py-4 rounded hover:bg-blue-800 flex items-center justify-center gap-2 mt-2 transition shadow-lg"
                >
          ...
          <div className={`${cores.card} p-8 rounded shadow-md w-full max-w-md border-t-4 border-[#1351b4] relative`}>
            <button
              ...
              className={`absolute top-4 right-4 p-2 rounded-full transition ${
                temaEscuro ? "bg-white/10 hover:bg-white/20 text-gray-100" : "bg-gray-100 hover:bg-gray-200 text-gray-700"
              }`}
            ...
            {modoSelecaoConta && !viewRecuperar ? (
                ...
                {contasSalvas.length > 3 && (
                  <div className="relative mb-4">
                    ...
                    <input
                      ...
                      className={`w-full pl-9 p-2 border rounded text-sm outline-none focus:border-blue-500 ${
                        temaEscuro ? "bg-[#2c2c2c] text-white border-gray-700" : "bg-gray-50"
                      }`}
              ...
              <div className="animate-fade-in">
                {contasSalvas.length > 0 && (
                  <button
                    ...
                    className={`mb-4 text-xs flex items-center gap-1 transition ${temaEscuro ? "text-gray-400 hover:text-blue-300" : "text-gray-500 hover:text-blue-600"}`}
                  >
                ...
                {(contaSelecionadaInfo || (email && contasSalvas.find((c) => c.email === email))) && (
                  <div
                    className={`mb-6 flex items-center gap-3 p-3 rounded-lg border ${
                      temaEscuro ? "bg-blue-500/10 text-blue-200 border-blue-500/20" : "bg-blue-50 text-blue-900 border-blue-100"
                    }`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${temaEscuro ? "bg-white/10" : "bg-blue-200 text-blue-800"}`}>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold leading-tight">{contaSelecionadaInfo?.nome || contasSalvas.find((c) => c.email === email)?.nome}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="relative mb-2">
                  <input
                    ...
                    className={`w-full p-3 border rounded focus:outline-none focus:ring-2 focus:ring-[#1351b4] ${cores.input} pr-10 ${
                      erroLogin ? "border-red-500 focus:ring-red-200" : ""
                    }`}
                  ...
                  <button
                    ...
                    className="absolute right-3 top-3 text-gray-400 hover:text-[#1351b4] transition"
                  >
                ...
                <div
                  className={`flex items-center gap-1.5 text-[10px] mb-4 p-2 rounded border ${
              ...
              <>
                ...
                {passoRecuperar === 1 ? (
                  <>
                    ...
                    <div className="relative mb-4">
                      ...
                      <input
                        className={`w-full pl-10 p-3 border rounded outline-none focus:border-blue-500 ${
                          temaEscuro ? "bg-[#2c2c2c] text-white border-gray-700" : "bg-white text-black border-gray-300"
                        }`}
                    ...
                    <button
                      ...
                      className="w-full bg-[#1351b4] text-white font-bold py-3 rounded hover:bg-[#0c3b85] transition shadow-md"
                    >
                      Enviar Código
                    </button>
                  </>
                  ...
                  <>
                    <div
                      className={`p-3 rounded mb-4 text-xs border flex items-start gap-2 ${
                        temaEscuro ? "bg-blue-500/10 text-blue-200 border-blue-500/20" : "bg-blue-50 text-blue-800 border-blue-100"
                      }`}
                    >
                      <Mail size={16} className="mt-0.5" />
                      <span>
                        Código enviado para <b>{emailRecuperar}</b>
                      </span>
                    </div>

                    <input
                      className={`w-full p-3 mb-3 border rounded outline-none ${
                        temaEscuro ? "bg-[#2c2c2c] text-white border-gray-700" : "bg-white text-black border-gray-300"
                      }`}
                    ...
                    <div className="relative mb-4">
                      ...
                      />
                      <button
                        onClick={() => setMostrarSenhaRec(!mostrarSenhaRec)}
                        className={`absolute right-3 top-3 transition ${temaEscuro ? "text-gray-400 hover:text-green-300" : "text-gray-400 hover:text-green-600"}`}
                      >
                    ...
                    <button
                      onClick={redefinirSenhaRecuperacao}
                      className="w-full bg-green-600 text-white font-bold py-3 rounded hover:bg-green-700 transition shadow-md"
                    >
                      Redefinir Senha
                    </button>
                  </>
                ...
                <button
                  ...
                  className={`w-full mt-4 text-sm border-t pt-4 transition ${
                    temaEscuro ? "text-gray-400 hover:text-gray-200 border-gray-700" : "text-gray-400 hover:text-gray-600 border-gray-200"
                  }`}
                >
                  Cancelar e Voltar
                </button>
  ...
  // ==========================================================
  return (
    <div className={`min-h-screen overflow-x-hidden font-sans ${cores.bg} ${cores.text} transition-colors duration-300`}>
      <header className={`${cores.header} text-white h-16 flex items-center px-4 md:px-8 justify-between shadow-md relative z-20`}>
        <div className="flex items-center gap-4 min-w-0">
          ...
          <span className="text-sm font-light hidden md:block truncate">
            Bem-vindo, {primeiroNome(user?.nome)}
          </span>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className={`${cores.subHeader} h-12 flex items-center px-4 md:px-8 text-white shadow-inner transition-colors relative z-10`}>
        <Menu className="w-5 h-5 mr-3 cursor-pointer hover:opacity-80 md:cursor-default" onClick={() => setMenuMobileAberto(!menuMobileAberto)} />
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 flex flex-col md:flex-row gap-6 relative">
        <>
          {menuMobileAberto && (
            <div className="fixed inset-0 bg-black/50 z-30 md:hidden animate-fade-in" onClick={() => setMenuMobileAberto(false)}></div>
          )}
          ...
          >
            <div className={`p-4 border-b ${cores.border} font-bold text-[#1351b4] flex items-center justify-between`}>
              ...
              <button onClick={() => setMenuMobileAberto(false)} className="md:hidden text-gray-500">
                <X size={20} />
              </button>
            </div>
            ...
            <nav>
              <div
                ...
                className={`p-3 px-4 flex items-center gap-3 text-sm cursor-pointer border-b ${cores.border} ${
                  view === "registro" ? cores.sideActive + " border-l-4 font-bold" : "hover:opacity-70 " + cores.textSec
                }`}
              >
              ...
              <div
                ...
                className={`p-3 px-4 flex items-center gap-3 text-sm cursor-pointer border-b ${cores.border} ${
                  view === "ficha" ? cores.sideActive + " border-l-4 font-bold" : "hover:opacity-70 " + cores.textSec
                }`}
              >
        ...
        <div className="flex-1 w-full min-w-0">
          ...
          {view === "registro" && (
            <div className="space-y-6">
              <div className={`${cores.card} rounded shadow-sm border ${cores.border} p-6 md:p-8 flex flex-col items-center justify-center min-h-[400px]`}>
                <div className="text-center mb-8">
                  <div className="flex items-center justify-center gap-2 text-[#1351b4] font-bold mb-1">
                    <Clock className="w-5 h-5" /> Hora atual
                  </div>
                  <div className={`text-5xl sm:text-6xl font-bold tracking-tight ${temaEscuro ? "text-white" : "text-gray-700"}`}>
                    {horaAtual.toLocaleTimeString("pt-BR")}
                  </div>
                  <div className={`${cores.textSec} mt-2 text-sm capitalize`}>
                    {horaAtual.toLocaleDateString("pt-BR", { weekday: "long", year: "numeric", month: "2-digit", day: "2-digit" })}
                  </div>
                </div>
                ...
                <div className="text-center w-full max-w-2xl">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 justify-center mb-8">
                    <BotaoSelecao
                      ...
                      ativo={tipoSelecionado === "Entrada"}
                      habilitado={verificarPermissao("Entrada")}
                      onClick={() => verificarPermissao("Entrada") && setTipoSelecionado("Entrada")}
                      temaEscuro={temaEscuro}
                      corPadrao="bg-[#f2ca4b]"
                    />
                    <BotaoSelecao
                      titulo="Ida Intervalo"
                      icone={<Coffee className="w-6 h-6" />}
                      ativo={tipoSelecionado === "Ida Intervalo"}
                      habilitado={verificarPermissao("Ida Intervalo")}
                      onClick={() => verificarPermissao("Ida Intervalo") && setTipoSelecionado("Ida Intervalo")}
                      temaEscuro={temaEscuro}
                      corPadrao="bg-blue-400"
                    />
                    <BotaoSelecao
                      titulo="Volta Intervalo"
                      icone={<ArrowLeftCircle className="w-6 h-6" />}
                      ativo={tipoSelecionado === "Volta Intervalo"}
                      habilitado={verificarPermissao("Volta Intervalo")}
                      onClick={() => verificarPermissao("Volta Intervalo") && setTipoSelecionado("Volta Intervalo")}
                      temaEscuro={temaEscuro}
                      corPadrao="bg-blue-500"
                    />
                    <BotaoSelecao
                      titulo="Saída"
                      icone={<LogOut className="w-6 h-6" />}
                      ativo={tipoSelecionado === "Saída"}
                      habilitado={verificarPermissao("Saída")}
                      onClick={() => verificarPermissao("Saída") && setTipoSelecionado("Saída")}
                      temaEscuro={temaEscuro}
                      corPadrao="bg-[#e6e6e6]"
                      textoEscuro={!temaEscuro}
                    />
                  </div>
              ...
              <div className={`${cores.card} p-6 rounded shadow-sm border ${cores.border}`}>
                <h3 className="text-[#1351b4] font-bold border-b pb-2 mb-4 flex items-center gap-2">
                  <Clock size={16} /> Registros do Turno (03h às 03h)
                </h3>
                ...
                <div className="flex flex-wrap items-end gap-4">
                  <div className="flex flex-col">
                    ...
                    <select
                      ...
                      className={`p-2 rounded border text-sm font-bold ${cores.input} min-w-[120px]`}
                    >
                  ...
                  <div className="flex flex-col">
                    ...
                    <select
                      ...
                      className={`p-2 rounded border text-sm font-bold ${cores.input} min-w-[100px]`}
                    >
...
function ItemDia({ dia, cores, temaEscuro, mensagemSalva, onSalvarMensagem }) {
  ...
  const ultimaSaida = dia.pontos.filter((p) => p.tipo === "Saída").at(-1);
    ...
    <div className={`border-b ${cores.border}`}>
      ...
      >
        <div className="flex flex-col min-w-0">
          ...
          <span className="text-[10px] border rounded-full px-2 py-0.5 mt-1 w-fit border-gray-300 text-gray-500 font-medium">
            {badgeTexto}
          </span>
        </div>
          ...
          {modoEdicaoMsg ? (
            <div className="max-w-md mx-auto bg-white p-4 rounded shadow border border-gray-200">
              ...
              <div className="flex justify-between gap-2">
                <button
                  onClick={() => setModoEdicaoMsg(false)}
                  className="flex-1 border border-gray-300 text-gray-600 py-2 rounded text-xs font-bold hover:bg-gray-100 flex items-center justify-center gap-1"
                >
                  <X size={14} /> Cancelar
                </button>
                <button
                  onClick={handleSalvar}
                  className="flex-1 bg-[#1351b4] text-white py-2 rounded text-xs font-bold hover:bg-blue-800 flex items-center justify-center gap-1"
                >
            ...
            <>
              {mensagemSalva && (
                <div
                  className="max-w-md mx-auto mb-4 bg-blue-50 border border-blue-200 p-3 rounded flex items-start gap-3 cursor-pointer hover:bg-blue-100 transition"
                  onClick={() => setModoEdicaoMsg(true)}
                >
              ...
              <div className={`max-w-md mx-auto rounded p-4 mb-4 ${temaEscuro ? "bg-[#333]" : "bg-[#ececec]"}`}>
                <div className="flex justify-between items-center text-lg font-mono font-bold text-gray-500">
                  <div className="text-center">
                    ...
                    <span className={primeiraEntrada ? "text-green-600" : ""}>
                  ...
                  <div className="text-center">
                    ...
                    <span className={ultimaSaida ? "text-red-600" : ""}>
...
function BotaoSelecao({ titulo, icone, ativo, habilitado, onClick, temaEscuro, corPadrao, textoEscuro }) {
  const baseClass = `group w-full md:w-32 h-24 md:h-28 rounded flex flex-col items-center justify-center shadow-sm transition-all border-2`;
  const disabledClass = !habilitado ? "opacity-30 cursor-not-allowed" : "cursor-pointer";
  ...
  if (!habilitado) {
    ...
    if (temaEscuro) {
      bgClass = ativo ? "bg-[#383838] text-white" : "bg-[#2c2c2c] text-white hover:bg-[#383838]";
    } else {
  ...
  return (
    <div onClick={habilitado ? onClick : undefined} className={`${baseClass} ${disabledClass} ${activeClass} ${bgClass}`}>
...
function gerarDiasDoMesSelecionado(mes, ano, historico) {
  ...
  // indexa histórico por dia local (YYYY-MM-DD)
  const mapPorDia = new Map();
  (historico || []).forEach((h) => {
    const key = localISODate(h.data);
    if (!mapPorDia.has(key)) mapPorDia.set(key, []);
    mapPorDia.get(key).push(h);
  });

  for (let i = 1; i <= ultimoDia; i++) {
    const d = new Date(ano, mes, i);
    const dataIso = localISODate(d);
    const pontosDoDia = mapPorDia.get(dataIso) || [];
